...
def find_out_position_extra(text):
    ...
    #这里是对抓出的数据的进一步处理
    position = position.replace("本公司", "").replace("其担任","").replace("所担任","").strip("的").replace("一职","").lstrip("公司").lstrip("其")
    if "所有" in position or "相关" in position or "一切" in position:
        position = position
        #print(position)
    else:
...
def find_in_position(text):
    ...
    position = position.replace("本公司", "").strip(" ").replace("一职", "").replace("新任", "").lstrip("公司").replace("新任","").strip("的")
    if "所有" in position or "相关" in position or "一切" in position:
        position = position
    else:
        position = position.replace("职务", "")
            ...
            for j in range(len(in_people)):
                if in_people[j].position in out_people[i].position and in_people[j].position !="" and in_people[j].name !="" and out_people[i].name !="":
                    ...
                    if i+1<len(out_people):
                        i+=1
                elif out_people[i].position in in_people[j].position and in_people[j].position !="" and in_people[j].name !="" and out_people[i].name !="":
                    union_people.append(people_union(out_people[i].name,out_people[i].sex,out_people[i].reason,out_people[i].position,in_people[j].name,in_people[j].sex,in_people[j].position))
                    in_people[j].name = ""
                    out_people[i].name = ""
                    if i+1<len(out_people):
...
#---------------------------------------------------------------------------------------------------------------------------
def save_json(filename,text,savepath):
    #这里是存储json文件，功能完好，勿动
    out_people, in_people, union_people = deal_texts(text)
    data = ""
    flag = 0
    data += '{"' + filename.split('.')[0].replace("_", "*") + '": {"证券代码": "' + filename.split('-')[0] + '", "证券简称": "' + \
           filename.split('-')[1].replace("_", "*") + '", "人事变动": ['
    if union_people !=[]:
    ...
    with open(savepath+filename[:-4]+".json", "w", encoding="utf-8") as f:
        f.write(js)
#---------------------------------------------------------------------------------------------------------------------------
#这里是读pdf文件
def read_pdf(rootdir):
    #该data_temp用以单个调试文档

    # readpath = rootdir + "Data_temp/"

    readpath = rootdir + "Data_read/"
    savepath = rootdir + "Data_save/"
    ...
    for parent, dirnames, filenames in os.walk(readpath):
        for filename in filenames:
            ...
            if (filename.endswith('pdf') or filename.endswith('PDF')):
                ...
                if "特此公告" in text:
                    temp_index = text.index("特此公告")
                    text = text[:temp_index]

                text = text.split("。")[1:-1]
                #deal_texts(text)
                #print("filename",filename)
                save_json(filename, text, savepath)
                fp.close()
                device.close()
                retstr.close()
